...
object Generator {
    ...
    fun newArt(enabled: List<Pattern>): ArtSpec {
        val rng = entropy()
        val pattern = rng.pick(enabled.ifEmpty { Patterns.all })
        val count = rng.int(pattern.colorRange)
        return ArtSpec(pattern.id, rng.nextLong(), PaletteGen.generate(count, rng))
    }
    ...
    fun switchColors(spec: ArtSpec): ArtSpec =
        spec.copy(colors = PaletteGen.generate(spec.colors.size, entropy()))
}
